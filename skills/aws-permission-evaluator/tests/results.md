...
| T9 SCP blocks vs RP grant | Denied | Denied (SCP must affirmatively allow; not bypassable) | yes | ✅ |
...
decision walk in SKILL.md, and the four tricky corners (implicit vs explicit deny, aws/s3
cross-account, cross-account both-sides, CMK enumeration) each have a dedicated rule and a
worked example.

## Gaps / future work

- These are manual traces. For regression safety, run the prompts through
  claude-with-the-skill using the skill-creator eval harness and score automatically.
- No negative-trigger tests yet (prompts that should NOT fire the skill, e.g. "write an IAM
  policy that grants s3:GetObject" — a generation task, not an evaluation task). Worth adding
  to confirm the description doesn't over-trigger.
